...
class AppMonitorModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    /**
     * P4/P5: Schedule an intention timer via SystemBrainBridge.
     * Persists untilMs in IntentionStore (authoritative) and schedules
     * a Handler callback that emits RuntimeEvent.TimerExpired.
     *
     * @param app Package name of the monitored app
     * @param durationMs Timer duration in milliseconds
     * @param promise Resolves with true on success
     */
    @ReactMethod
    fun scheduleIntentionTimer(app: String, durationMs: Double, promise: Promise) {
        try {
            val durationLong = durationMs.toLong()
            if (durationLong <= 0) {
                promise.reject("INVALID_DURATION", "Duration must be > 0")
                return
            }
            SystemBrainBridge.scheduleIntentionTimer(app, durationLong)
            promise.resolve(true)
        } catch (e: Exception) {
            android.util.Log.e("AppMonitorModule", "Failed to schedule intention timer", e)
            promise.reject("SCHEDULE_TIMER_FAILED", e.message, e)
        }
    }
